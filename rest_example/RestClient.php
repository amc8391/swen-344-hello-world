...
<?php
// $apiSrc = 'http://www.se.rit.edu/~amc8391/rest_example/RESTAPI.php';
$apiSrc = 'http://www.se.rit.edu/~hjm3628/RestActivity/RESTAPI.php';
$clientSrc = 'http://www.se.rit.edu/~amc8391/rest_example/RestClient.php';
// Author: Daniel Krutz
...
{
  $person_info = file_get_contents($apiSrc . '?action=get_person&id=' . $_GET["id"]);
  ...
  ?>
    ...
    <a href=<?php echo $clientSrc . "?action=get_person_list" ?> alt="person list">Return to the person list</a>
  <?php
}
else // else take the person list
{
  ...
  $person_list = file_get_contents($apiSrc . '?action=get_person_list');
  ...
  ?>
    ...
    <?php foreach ($person_list as $person): ?>
      <li>
        ...
        // <a href=<?php echo $clientSrc . "?action=get_person&id=" . $person["id"]  ?> alt=<?php echo "person_" . $person_["id"] ?>><?php echo $person["name"] ?></a>
    </li>
    <?php endforeach; ?>
    </ul>
  <?php
